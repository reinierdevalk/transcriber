...
def parse_args():  # -> None:
    ...
    parser.add_argument(
        "-x", "--accidentals", choices=[YES, NO], default=NO, metavar="", help=""
    )
    parser.add_argument(
        "-m", "--mode", choices=[MAJOR, MINOR], default=MAJOR, metavar="", help=""
    )
    parser.add_argument(
        "-u",
        "--tuning",
    ...
    parser.add_argument(
        "-t", "--tablature", choices=[YES, NO], default=YES, metavar="", help=""
    )
    parser.add_argument(
        "-y",
        "--type",
    ...
    parser.add_argument(
        "-p", "--placement", choices=[TOP, BOTTOM], default=BOTTOM, metavar="", help=""
    )
    parser.add_argument(
        "-c", "--custom", choices=["dlaute", "plaute", "user"], metavar="", help=""
    )
    ...
    if error_li:
        print(
            "\n\n".join(
                [
                    (
                        f"While transcribing {file} the following Error occured. \n{err}\n"
                        f"Please create an Issue with the Error message and the problematic file at "
                        "https://github.com/reinierdevalk/abtab/issues"
                    )
                    for file, err in error_li
                ]
            )
        )
