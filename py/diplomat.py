...
from py.utils import (
    ...
    handle_namespaces,
    parse_tree,
    get_main_MEI_elements,
    collect_xml_ids,
    unwrap_markup_elements,
    print_all_elements,
    pretty_print,
    get_isodate,
)
...
def make_element(
    name: str, parent: ET.Element = None, atts: list = []
):  # -> ET.Element:
    """
...
# Main functions -->
def handle_encodingDesc(
    encodingDesc: ET.Element, ns: dict, args: argparse.Namespace
):  # -> None:
    """
    ...
    application = make_element(
        f"{URI_MEI}application",
        atts=[
            (XML_ID_KEY, add_unique_id("a", XML_IDS)[-1]),
            ("isodate", get_isodate()),
    ...
    ET.SubElement(
        application, f"{URI_MEI}p", **{f"{XML_ID_KEY}": add_unique_id("p", XML_IDS)[-1]}
...
def handle_scoreDef(
    scoreDef: ET.Element, ns: dict, args: argparse.Namespace
):  # -> None:
    """
    ...
    if args.tablature == YES:
        ...
        if TYPE != NOTATIONTYPES[GLT]:
            tab_staffDef.set(
                "lines", "5" if lines == "5" and TYPE == NOTATIONTYPES[FLT] else "6"
            )
    ...
    nh_staffGrp = ET.Element(
        f"{URI_MEI}staffGrp", **{f"{XML_ID_KEY}": add_unique_id("sg", XML_IDS)[-1]}
    )
        ...
        if is_first_scoreDef:
            if args.score == SINGLE:
                clef = make_element(
                    ...
                    parent=nh_staffDef,
                    atts=[
                        (XML_ID_KEY, add_unique_id("c", XML_IDS)[-1]),
                        ("shape", "G"),
                        ("line", "2"),
                        ("dis", "8"),
                        ("dis.place", "below"),
...
def copy_and_transform_staff(elem: ET.Element, parent_map: dict):  # -> ET.Element:
    ...
    new_elem = (
        make_new_element(elem, parent_map)
        if elem.tag in elems_to_replace
        else ET.Element(elem.tag)
    )
    # If elem is a <tabGrp>
    ...
    if elem.tag not in elems_to_replace:
        ...
        for attr, val in elem.attrib.items():
            new_elem.set(
                ...
                (
                    val
                    if attr != f"{URI_XML}id"
                    else add_unique_id(elem_name[0], XML_IDS)[-1]
                ),
            )
    ...
    for child in elem:
        ...
        if child.tag == f"{URI_MEI}beam":
            for grandchild in child:
                new_grandchild, chords_grandchild, notes_grandchild = (
                    copy_and_transform_staff(grandchild, parent_map)
                )
        ...
        new_child, chords_child, notes_child = copy_and_transform_staff(
            child, parent_map
        )
    ...
    if elem.tag == f"{URI_MEI}tabGrp":
        ...
        # If elem contains no notes: new_elem is a <rest>
            ...
            new_elem = make_element(
                f"{URI_MEI}rest",
                atts=[
                    (XML_ID_KEY, add_unique_id("r", XML_IDS)[-1]),
                    ("dur", elem.get("dur")),
                ],
            )
    elif elem.tag == f"{URI_MEI}note":
        try:
            midi_pitch = _get_midi_pitch(
                int(elem.get("tab.course")), int(elem.get("tab.fret")), TUNING
            )
        except TypeError:
            raise Exception(
                f"Element {elem.tag} with attributes\
							{elem.attrib} is missing tab.course or tab.fret"
            )
        new_elem = make_element(
            f"{URI_MEI}note",
            atts=[
                (XML_ID_KEY, add_unique_id("n", XML_IDS)[-1]),
...
# NEW!
def find_ancestor_with_tag(
    elem: ET.Element, parent_map: dict, tag: str
):  # -> ET.Element | None:
    """
...
# NEW!
def handle_section_NEW(
    section: ET.Element, ns: dict, args: argparse.Namespace
):  # -> tuple:
    ...
    staffs_and_dirs = (
        []
    )  # contains, per measure: old staff, new staff, combined dirs (flag dirs + other)
    parent_map = {
        c: p for p in section.iter() for c in p
    }  # this is a *reverse* (child -> parent) lookup dict!
    tab_to_CMN_note_ids = {}
        ...
        if elem.tag == f"{URI_MEI}measure":
            ...
            for melem in [child for child in elem]:
                ...
                if melem.tag == f"{URI_MEI}staff":
                    ...
                    new_staff, chords, notes = copy_and_transform_staff(
                        melem, parent_map
                    )
                    new_staff.set("n", "1")
                    ...
                    for r in new_staff.findall(".//mei:rest", ns):
                        flag_dir = _make_rhythm_symbol_dir(
                            r.get(XML_ID_KEY), r.get("dur"), r.get("dots"), ns
                        )
                    ...
                    for c in new_staff.findall(".//mei:chord", ns):
                        flag_dir = _make_rhythm_symbol_dir(
                            c.get(XML_ID_KEY), c.get("dur"), c.get("dots"), ns
                        )
                        ...
                        for el in list(c):
                            ...
                            # tabDurSym is placed inside wrapper
                                ...
                                el_for_flag_dirs.set(
                                    XML_ID_KEY,
                                    add_unique_id(get_element_name(el)[0], XML_IDS)[-1],
                                )
                    ...
                    for n in list(notes.values()):
                        notes_unspelled_by_ID.append(
                            [n.get(XML_ID_KEY), measure, n.get("pname")]
                        )
                ...
                # 2. Copy/transform any other element that is a direct child of <measure>
                    ...
                    if any(e.tag == f"{URI_MEI}dir" for e in melem.iter()):
                        ...
                        dirs = (
                            [melem]
                            if melem.tag == f"{URI_MEI}dir"
                            else melem.findall(".//mei:dir", ns)
                        )
                        for d in dirs:
                            d.set("staff", tab_staff_n)
                        dirs_CMN = (
                            [melem_CMN]
                            if melem_CMN.tag == f"{URI_MEI}dir"
                            else melem_CMN.findall(".//mei:dir", ns)
                        )
                        for d in dirs_CMN:
                            d.set("staff", "1")
                    ...
                    if any(e.tag == f"{URI_MEI}fing" for e in melem.iter()):
                        ...
                        fings_CMN = (
                            [melem_CMN]
                            if melem_CMN.tag == f"{URI_MEI}fing"
                            else melem_CMN.findall(".//mei:fing", ns)
                        )
                        for f in fings_CMN:
                    ...
                    if any(e.tag == f"{URI_MEI}fermata" for e in melem.iter()):
                        ...
                        fermatas_CMN = (
                            [melem_CMN]
                            if melem_CMN.tag == f"{URI_MEI}fermata"
                            else melem_CMN.findall(".//mei:fermata", ns)
                        )
                    ...
                    for e in melem_CMN.iter():
                        e.set(
                            XML_ID_KEY,
                            add_unique_id(get_element_name(e)[0], XML_IDS)[-1],
                        )
                    # Add to other
...
def handle_section(
    ...
    for measure in section.iter(f"{URI_MEI}measure"):
        ...
        non_regular_elements = [
            elem for elem in measure.iter() if elem.tag not in regular_elements
        ]
        # Collect
        ...
        tab_staff.set(
            "n", str(int(tab_staff.attrib["n"]) + (1 if args.score == SINGLE else 2))
        )
        ...
        nh_staff_1 = ET.Element(
            f"{URI_MEI}staff",
            **{f"{XML_ID_KEY}": add_unique_id("s", XML_IDS)[-1]},
            n="1",
        )
        nh_staff_2 = ET.Element(
            f"{URI_MEI}staff",
            **{f"{XML_ID_KEY}": add_unique_id("s", XML_IDS)[-1]},
            n="2",
        )
        ...
        nh_layer_1 = ET.SubElement(
            nh_staff_1,
            f"{URI_MEI}layer",
            **{f"{XML_ID_KEY}": add_unique_id("l", XML_IDS)[-1]},
            n="1",
        )
        nh_layer_2 = ET.SubElement(
            nh_staff_2,
            f"{URI_MEI}layer",
            **{f"{XML_ID_KEY}": add_unique_id("l", XML_IDS)[-1]},
            n="1",
        ...
        for tabGrp in tab_layer.iter(f"{URI_MEI}tabGrp"):
            ...
            rest_case_2a = (
                flag == None
                and rest != None
                and rest.get("glyph.name") in SMUFL_LUTE_DURS.values()
            )
            rest_case_2b = (
                flag == None
                and rest != None
                and rest.get("glyph.name") not in SMUFL_LUTE_DURS.values()
            )
            ...
            if rest_case_1 or rest_case_2a or rest_case_2b or rest_case_3:
                ...
                space_1 = make_element(
                    f"{URI_MEI}space",
                    parent=nh_layer_1,
                    atts=[(XML_ID_KEY, xml_id_space_1), ("dur", dur)],
                )
                space_2 = make_element(
                    f"{URI_MEI}space",
                    parent=nh_layer_2,
                    atts=[(XML_ID_KEY, xml_id_space_2), ("dur", dur)],
                )

                # 2. Add <dir>
                if not rest_case_2b:
                    rhythm_symbol_dirs.append(
                        _make_rhythm_symbol_dir(xml_id_space_1, dur, dots, ns)
                    )
            ...
            # Add <chord>s and/or <space>s
                ...
                chord_1 = make_element(
                    f"{URI_MEI}chord",
                    atts=[
                        (XML_ID_KEY, xml_id_chord_1),
                        ("dur", dur),
                        ("stem.visible", "false"),
                    ],
                )
                chord_2 = make_element(
                    f"{URI_MEI}chord",
                    atts=[
                        (XML_ID_KEY, xml_id_chord_2),
                        ("dur", dur),
                        ("stem.visible", "false"),
                    ],
                )
                for element in tabGrp:
                    if element != flag and element != rest and element != space:
                        ...
                        nh_note = make_element(
                            f"{URI_MEI}note",
                            parent=(
                                chord_1
                                if args.score == SINGLE
                                else (chord_1 if midi_pitch >= 60 else chord_2)
                            ),
                            atts=(
                                [
                                    (XML_ID_KEY, xml_id_note),
                                    ("pname", None),
                                    ("oct", str(_get_octave(midi_pitch))),
                                    ("head.fill", "solid"),
                                ]
                                if midi_pitch != -1
                                else [(XML_ID_KEY, xml_id_note), ("visible", "false")]
                            ),
                        )
                        # Map tab <note>
                        tab_notes_by_ID[element.get(XML_ID_KEY)] = (element, nh_note)
                        notes_unspelled_by_ID.append(
                            [xml_id_note, measure.get("n"), midi_pitch]
                        )

                # 1. Add <chord>s and/or <space>s to <layer>s
                xml_id_space = add_unique_id("s", XML_IDS)[-1]
                nh_space = make_element(
                    f"{URI_MEI}space", atts=[(XML_ID_KEY, xml_id_space), ("dur", dur)]
                )
                ...
                if flag != None:
                    rhythm_symbol_dirs.append(
                        _make_rhythm_symbol_dir(xml_id_reference, dur, dots, ns)
                    )
                ...
                chords = (
                    ...
                    if args.score == SINGLE
                        ...
                        chord_1 if len(chord_1) > 0 else nh_space,
        ...
        for c in elems_removed_from_measure:
            ...
            if c.tag == f"{URI_MEI}fermata":
                ...
                rhythm_symbol_dirs.append(
                    _make_rhythm_symbol_dir(xml_id_upper_chord, "f", None, ns)
                )
            ...
            # Annotation: needs <annot> (CMN) and <annot> (= c; tab)
                ...
                xml_ids_referred = [
                    xml_id.strip("#") for xml_id in c.get("plist").split()
                ]  # split plist into xml_ids, remove #
                ...
                xml_ids_tab_elem = []
                for elem_referred, xml_id_referred in zip(
                    elems_referred, xml_ids_referred
                ):
                    if elem_referred.tag in tab_elements:
                        contains_tab_elem = True
                        if (
                            elem_referred.tag == f"{URI_MEI}note"
                            or elem_referred.tag == f"{URI_MEI}rest"
                        ):
                            xml_ids_tab_elem.append(
                                tab_notes_by_ID[xml_id_referred][1].get(XML_ID_KEY)
                            )
                        elif elem_referred.tag == f"{URI_MEI}tabGrp":
                            xml_ids_tab_elem.append(
                                tabGrps_by_ID[xml_id_tabGrp][1][0].get(XML_ID_KEY)
                            )
                ...
                if contains_tab_elem:
                    ...
                    annot.set(
                        "plist", " ".join(["#" + xml_id for xml_id in xml_ids_tab_elem])
                    )
            ...
            # (Text) directive: needs <dir> (CMN) and <dir> (= c; tab)
                ...
                for e in direc.iter():
                    e.set(
                        XML_ID_KEY,
                        add_unique_id(remove_namespace_from_tag(e.tag)[0], XML_IDS)[-1],
                    )
                    ...
                    if "startid" in e.attrib:
                        ...
                        xml_id_upper_chord = tabGrps_by_ID[xml_id_tabGrp][1][0].get(
                            XML_ID_KEY
                        )
        ...
        fermatas = [
            e for e in curr_non_regular_elements if e.tag == f"{URI_MEI}fermata"
        ]
...
def _make_rhythm_symbol_dir(
    xml_id: str, dur: int, dots: int, ns: dict
):  # -> ET.Element:
    d = ET.Element(
        f"{URI_MEI}dir",
    ...
    if dur != "f":
        make_element(
            ...
            parent=d,
            atts=[
                (XML_ID_KEY, add_unique_id("s", XML_IDS)[-1]),
                ("glyph.auth", "smufl"),
...
def spell_pitch(
    section: ET.Element, notes_unspelled_by_ID: list, args: argparse.Namespace
):  # -> None:
    # Dictionary for fast lookup of xml:ids
    xml_id_map = {
        e.get(XML_ID_KEY): e for e in section.iter() if XML_ID_KEY in e.attrib
    }

    grids_dict = call_java(
        ["java", "-cp", args.classpath, JAVA_PATH, args.dev, "grids", KEY, args.mode]
    )
    ...
    key_sig_accid_mpc = [
        mpcGrid[i] for i in range(len(altGrid)) if altGrid[i] == key_sig_accid_type
    ]
    ...
    cmd = [
        ...
        args.dev,
        "pitch",
        json.dumps(notes_unspelled_by_ID),
        KEY,
        json.dumps(mpcGrid),
        json.dumps(altGrid),
        json.dumps(pcGrid),
    ...
    for key, val in spell_dict.items():
        ...
        accidsInEffect = val[
            "accidsInEffect"
        ]  # double flats, flats, naturals, sharps, double sharps

        # a. The note is in key	and there are no accidentals in effect
        no_aie = not any(
            p for s1 in accidsInEffect for s2 in s1 for p in s2
        )  # True if accidsInEffect is completely empty
...
def transcribe(in_path: str, out_path: str, args: argparse.Namespace):  # -> None:
    ...
    filename, ext = os.path.splitext(
        in_file
    )  # in_file is already basename (see method call in transcriber.py)
